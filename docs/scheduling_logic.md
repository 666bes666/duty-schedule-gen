...
5. **Ограничения равномерности** — 5/2 без выходных, не более 6 дней подряд
...
- Не превышен лимит 6 рабочих дней подряд
...
### Фаза 2.5: Резервный дежурный

**Кто участвует:** Московские дежурные сотрудники.

**Условие:** В будние дни (не праздники), если среди московских дежурных нет ни одного на WORKDAY (помимо утреннего и вечернего дежурных), один назначается принудительно.

**Цель:** Обеспечить минимальное присутствие дежурного на рабочем месте для оперативного реагирования.
...
Если у московского дежурного достигнут лимит выходных подряд (`MAX_CONSECUTIVE_OFF`, не строгий) и он технически доступен:
- Назначается утренняя смена (если не после вечера и не `evening_only`)
- Иначе вечерняя смена (если не `morning_only`)

Лимит `MAX_CONSECUTIVE_OFF = 100` — не строгое ограничение, фактически серия выходных не лимитируется жёстко.
...
13. **Enforcement нормы** — снятие лишних WORKDAY с конца месяца (без проверки резервного дежурного)
14. **Assertion** — `ScheduleError` если `actual != target` для любого сотрудника

## Жёсткие инварианты

Следующие инварианты обязаны выполняться после генерации расписания. Нарушение любого из них — `ScheduleError`:

1. **Норма рабочих дней** — каждый сотрудник отрабатывает ровно `effective_target` дней. Постобработка не может нарушить норму: финальный enforcement снимает лишние WORKDAY, а assertion проверяет точное соответствие.
2. **Покрытие смен** — каждый день покрыт утренней, вечерней и ночной сменами.
