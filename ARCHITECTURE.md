...
> Полное описание системы для воспроизведения с нуля. Актуально на момент релиза v1.3.0.
...
Структура:
- Строки = дни месяца
- Столбцы = сотрудники (сгруппированы по городу: сначала Москва, потом Хабаровск)
- Ячейки = тип смены (Утро / Вечер / Ночь / Рабочий день / Выходной / Отпуск)
- Цветовое кодирование: каждый тип смены — свой цвет
- Праздники выделены отдельным фоном
- Итоговые строки: число рабочих дней, норма, дельта
