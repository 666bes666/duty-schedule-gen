...
MAX_CONSECUTIVE_OFF          = 100 # Макс. выходных подряд (не строгий лимит)
...
| Макс. выходных подряд | Не ограничено строго (100) |
...
- Норма рабочих дней — жёсткий инвариант. После всей постобработки выполняется финальный enforcement: лишние WORKDAY снимаются с конца месяца. Если норму невозможно соблюсти — `ScheduleError`
...
`fetch_holidays()` возвращает `tuple[set[date], set[date]]` — `(holidays, short_days)`. `short_days` используется при расчёте часов: 7ч вместо 8ч.
