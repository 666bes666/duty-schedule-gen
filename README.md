...
- Экспорт в `.ics`: отдельные файлы по типам смен (CLI) и персональный файл для каждого сотрудника (UI)
...
- Статистика и метрики качества расписания в отдельной вкладке
- Подсчёт часов в XLS (8 ч / 7 ч сокращённый день)
...
- Импорт carry-over из XLS предыдущего месяца (автоопределение состояния сотрудников на стыке месяцев)
...
Основные вкладки результата:

- **Календарь** — расписание в виде таблицы, строки отсортированы по городу, признаку дежурства, типу графика и имени
- **Нагрузка** — статистика: покрытие по дням и распределение типов смен
- **Экспорт** — скачать XLS и персональный `.ics`-файл для каждого сотрудника
...
# Тесты
no_proxy="*" uv run pytest tests/ -q --ignore=tests/ui

# Тесты с покрытием
no_proxy="*" uv run pytest --cov=duty_schedule --cov-report=html --ignore=tests/ui
```
